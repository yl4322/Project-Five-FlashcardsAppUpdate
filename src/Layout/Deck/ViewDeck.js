...
function Deck(){
const [deck, setDeck] = useState({})  // the diff between {} and [] - map function only works for array bracket
 ...
 return(
     <div>
        ...
        <div>
            ...
            <button className="btn btn-danger mx-4" onClick={()=>handleDeleteDeck(deckId)} >Delete</button>
            ...
            {cards.map((card, index)=>{
                return <div className="card" key={index}>
                        <div className="card-body">
                            ...
                            <Link to={`./${deckId}/cards/${card.id}/edit`} className="btn btn-secondary mx-1">Edit</Link>
                            <button className="btn btn-danger mx-3" onClick={()=>handleDeleteCard(card.id)}>Delete</button>                        </div>
                    </div>
